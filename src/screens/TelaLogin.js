...
export default function TelaLogin({navigation}) {
  ...
  async function Cadastrar(cpf) {
    //console.log(String(nomeCliente).length, String(nomeCliente).search(" "));
    if (
      validaCPF(cpf) &&
      String(nomeCliente).length >= 8 &&
      String(nomeCliente).search(' ') != -1
    ) {
      ...
      setModal(true);
      //alert(`${String(nomeCliente).length >= 8 && nomeCliente.search(" ") != -1 ? '' : 'Nome inválido, insira nome e sobrenome, Ex: Maria Silva.'}
    ...
    if (validaEmail(emailCliente) && validaSenha(senhaCliente)) {
      try {
        const response = await api.get('/UserLogin/', {
          params: {emailCliente: emailCliente, senhaCliente: senhaCliente},
        });
        const data = response.data.response;

        if (data != undefined) {
          data.map(item => {
            (valida = item.idLoginCliente), (idClientee = item.idCliente);
          });
        }
        if (valida != 0) {
          pegarDadosCliente();
  ...
  async function pegarDadosCliente() {
    try {
      const response = await api.get('/User/', {
        params: {idCliente: idClientee},
      });
      const responseFone = await api.get('/UserPhone/', {
        params: {idCliente: idClientee},
      });
      const data = response.data.response;
      ...
      data.map(item => {
        (nomeClientee = item.nomeCliente), (cpfClientee = item.cpfCliente);
      });
      ...
      <KeyboardAvoidingView style={styles.fundo}>
        ...
        <View style={styles.formContainer}>
            ...
            <>
              ...
              <View style={styles.inputContainer}>
                ...
                <TextInputMask
                  ...
                  onSubmitEditing={() =>
                    stageNew ? Logar() : Cadastrar(cpfCliente)
                  }
                />
              </View>
            </>
          ...
          <View style={styles.bt}>
            <TouchableOpacity
              onPress={() => (stageNew ? Logar() : Cadastrar(cpfCliente))}>
              <View style={styles.btLogar}>
                <Text style={styles.txtLogin}>
                  {' '}
                  {stageNew ? 'Logar' : 'Prosseguir'}{' '}
                </Text>
              </View>
            </TouchableOpacity>
          </View>
          <TouchableOpacity style={styles.bt2} onPress={stageFunction}>
            <Text style={styles.txtCadastro}>
              {' '}
              {stageNew ? 'Não possui cadastro?' : 'Já possui cadastro?'}{' '}
            </Text>
          </TouchableOpacity>
        </View>
      </KeyboardAvoidingView>
          ...
          <View style={styles.Modal}>
            ...
            <View style={styles.AvisoModal}>
              <Text style={styles.AvisoTxt}>{`${
                String(nomeCliente).length >= 8 && nomeCliente.search(' ') != -1
                  ? ';)'
